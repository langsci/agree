...
\abstract{In DPs of some languages, possessors and external arguments of nouns that have them surface in low positions introduced by `of'. In others, this is impossible or highly restricted. I argue that nP-internal possessors/EAs give rise to the [XP, YP] configuration that thwarts the labeling algorithm of \citet{Chomsky2013, Chomsky2015}, forcing them to raise to Spec of an agreeing head such as D. But shared features of gender-number/noun class concord on `of' can label nP with possessor or EA in situ. In addition to explaining contrasting word orders this analysis provides novel evidence that concord is a syntactic phenomenon: it feeds labeling, and bleeds possessor/EA-raising in DP.}
...
\protectedex{
	\begin{exe}
		\ex Chichewa \citep[372, 374]{Carstens1997} \label{ex:3}
		\xlist
		\ex \label{ex:3a}
...
\begin{exe}
	\exbox{ \label{ex:5}
		...
		[\textit{n'}
		[\textit{n}]
		[NP\\ \textit{head noun}\\\textit{(possessum or argument-taking}\\ \textit{nominal})]
		] ] ] ] ] ]		
		\end{forest}}
\end{exe}
\end{figure}
...
\begin{exe}
		...
		7-picture   7-nice      7-my\\
		\ex \label{ex:7f}
		\gll \scalebox{0.9}{[\textsubscript{DP} N+n+Num+D [\textsubscript{NumP} changa <Num> [\textsubscript{nP} AP [\textsubscript{nP}  <changa> <N+\textit{n}>...]]]]}\\
...
\protectedex{
\begin{exe}
		\ex Swahili \citep[100]{Carstens1991} \label{ex:8}
		\xlist
		\ex 
...
\begin{exe} \settowidth\jamwidth{\textit{in situ} poss acquires noun class concord}
	...
	\glt `Lucy’s picture'  	
	\ex\label{ex:24b} \mbox{\textit{pre-concord}:   [\textsubscript{$\alpha$} [of Lucy] [\textit{n}\textsubscript{7} [ picture\textsubscript{7}]]]}\\ \jambox{labelling impossible} 
	\ex\label{ex:24c} \mbox{\textit{post-concord}:   [\textsubscript{$\alpha$} [\textsubscript{agr7} of Lucy] [\textit{n}\textsubscript{7} [picture\textsubscript{7}]]]}\\ \jambox{\textit{in situ} poss acquires noun class concord \newline (realized on 'of')}
	\ex\label{ex:24d} \mbox{\textit{post-labelling:} [\textsubscript{C7P} [\textsubscript{agr7} of Lucy] [\textit{n}\textsubscript{7} [picture\textsubscript{7}]]]}\\ \jambox{shared features label $\alpha$ C7P}
	\endxlist
\end{exe}
\begin{exe} 
	\ex \label{ex:25}
...
7picture-1S 7agr-my\\ \jambox{(pseudo-Chichewa)}
\glt `my picture'
\endxlist
\end{exe}
...
\exbox{
	...
	\begin{forest} for tree={fit=band}
		[PossP
		[DP
		[altʃani\\tree.\textsc{m.sg}, name=DP]
		[, nice empty nodes] ]
		[Poss'
		...
		\draw[-] (Poss) --++(0em,-7.5ex)--++ (-7.4em,0pt) --++(0em,0.3ex)--++ (DP);
		\node at (-1.3,-4){\textit{\textbf{Agree $\#$2}}};
	\end{forest}}
\end{exe}
\end{figure}
\newpage\noindent 
In Brinson's account, by the time the number head associated with the possessed noun is merged, the features of PAM have already been valued (see (\ref{ex:47})).
	...
	\begin{exe}
		\exbox{ \label{ex:48}
			...
			\begin{forest} 
				[DP1
				[NP
				[i-mbene-k\\leaf.\textsc{f}.\textsc{pl}]
				[, nice empty nodes] ]
				[D'
				[D [ɛ, name=D] ]
				[NumP
				...
				[PossP
				[DP2
				[\hspace{0.8cm}altʃani\\\hspace{1.2cm}tree.\textsc{m.sg}]
				[, nice empty nodes] ]
				[Poss'
				[Poss \textsubscript{\underline{S u}\sout{Num}, \underline{F \sout{u}}\sout{Gen}}, name=Poss]
				[NP
				...
				\draw[->] (Poss) --++(0em,-8ex)--++ (-9.24em,0pt) --++(0em,12.9ex)--++ (Num);
				\node at (1.4,-7.7){\textit{raising Poss-to-Num-to-D}};
		   		\draw[->] (Num) --++(0em,-4ex)--++ (-2.6em,0pt) --++(0em,8.7ex)--++ (D);
		\end{forest}}
	\end{exe}
\end{figure}
\newpage\noindent
	...
	\begin{exe}
		\exbox{ \label{ex:50}
			...
			\begin{forest}
				...
				\draw[->] (Poss) --++(0em,-8ex)--++ (-9.25em,0pt) --++(0em,12.9ex)--++ (Num);
				\node at (1.4,-7.7){\textit{raising Poss-to-Num-to-D}};
				\draw[->] (Num) --++(0em,-4ex)--++ (-3.2em,0pt) --++(0em,8.9ex)--++ (D);
		\end{forest}}
	\end{exe}
\end{figure}
...
\begin{exe}
	\exr{ex:12} \citep[301]{Newman2000}
	\xlist
	\ex 
	\gll \underline{riga}   bak’a   \textbf{\underline{t}a}       \textbf{Lawan} \\
	gown   black   of.fem    Lawan\\
...
\protectedex{
\begin{exe}
\ex\label{ex:56} COLOR > NATIONALITY/ORIGIN	\hspace{0.2cm}NATIONALITY/ORIGIN > COLOR  
\xlist 
\begin{multicols}{2} \raggedcolumns

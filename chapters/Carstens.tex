...
\begin{exe}
\ex \xlist
	\ex {\lbrack}\textsubscript{$\alpha$} buy [\textsubscript{$\beta$} a [\textsubscript{$\gamma$} \textit{n} book]]]      \textit{is labeled}
	\ex {\lbrack}\textsubscript{VP} buy [\textsubscript{DP} a [\textsubscript{nP} \textit{n} book]]{\rbrack}
	\endxlist
\end{exe}
...
\begin{exe}
	...
	\ex \mbox{\textit{post-concord}:   [\textsubscript{$\alpha$} [\textsubscript{agr7} of Lucy] [\textit{n}\textsubscript{7} [picture\textsubscript{7}]]] \jambox{\textit{in situ} poss acquires noun class concord\\ (realized on 'of')}}
...
\end{multicols}
	\exbox{
	\gll c.  \hspace{0.2cm}chi-tunzi    ch-abwino   ch-a Lucy\\
	{} \hspace{0.2cm}7-picture   7-nice           7-of 1Lucy\\
	\glt \hspace{0.5cm} `Lucy's nice picture' 
}
	\endxlist
\end{exe}
\end{figure}
...
\exbox{ 
	\begin{forest} 
		...
		\node at (4,0){$\rightarrow$ $\phi$P (Cl7P)};
		\node at (4,-2){$\rightarrow$ nP\textsubscript{7}};
		\node at (4,-3){$\rightarrow$ NP\textsubscript{7}}; 		
\end{forest}}
\end{multicols}
\endxlist
\end{exe}
...
\begin{figure}[!h]
	\begin{exe}
		...
		\forestset{nice empty nodes/.style={for tree={calign=fixed edge angles}, delay={where content={}{shape=coordinate, for current and siblings={anchor=north}}{}}
			...
			\exbox{ 
				\begin{forest} for tree={square, draw}
					[KP, nice empty nodes 
					[K\\uPhi\ul{}, name=K]	
					[PP
					[P]

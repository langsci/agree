...
\endxlist
\end{exe}
Assuming this analysis is correct, similar effects should be discernible in any syntactic domain where comparable configurations arise. An important domain to consider is the extended nominal projection in the sense of \citet{Grimshaw19912005}, henceforth DPs. Possessors and, for nouns that have them, EAs\footnote{On this issue see brief remarks and citations in section 1.4.} have been argued to merge as specifiers of \textit{n}, a nominal counterpart to \textit{v} within the DP (for possessors this projection is sometimes labeled PossP; for expository convenience I treat both cases alike): 
...
\begin{exe}
	\exbox{
		...
		[\textit{n'}
		[\textit{n}]
		[NP\\ \textit{head noun}\\\textit{(possessum or argument-taking nominal)}]
		] ] ] ] ] ]		
		\end{forest}}
\end{exe}
\end{figure}
...
\begin{exe} \settowidth\jamwidth{\textit{in situ} poss acquires noun class concord}
	...
	\ex \mbox{\textit{post-concord}:   [\textsubscript{$\alpha$} [\textsubscript{agr7} of Lucy] [\textit{n}\textsubscript{7} [picture\textsubscript{7}]]]}\\ \jambox{\textit{in situ} poss acquires noun class concord \newline (realized on 'of')}
...
\endxlist
\end{exe}
%%'my' should not be in the square, but it seems impossible to exclude it
...
\protectedex{
\begin{exe}
\ex COLOR > NATIONALITY/ORIGIN	\hspace{0.2cm}NATIONALITY/ORIGIN > COLOR  
\xlist 
\begin{multicols}{2} \raggedcolumns
